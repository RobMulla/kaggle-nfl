...
def calculate_3_closest(play):
    ...
    for time, d in play.groupby('time'):
        for role1, r1data in d.groupby(['role', 'gsisid']):
            ...
            min_dist = 2  # Large number greater thank any possible distance
...
for row in tqdm(vr.iterrows()):
    ...
    play_danger_df.to_csv('../working/playlevel/momentum/{}-{}-{}-alldata.csv'.format(year, gamekey, playid))

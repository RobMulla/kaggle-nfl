...
for row in tqdm(pi.iterrows()):
    ...
    try:
        ...
        play_danger_df.to_csv('../working/playlevel/momentum/closest/{}-{}-{}-lessthan1y.csv'.format(year, gamekey, playid))
        play_danger_df.to_parquet('../working/playlevel/momentum/closest/{}-{}-{}-lessthan1y.parquet'.format(year, gamekey, playid))
        end = timer()
        print(end - start)

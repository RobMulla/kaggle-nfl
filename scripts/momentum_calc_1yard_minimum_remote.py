...
remote_dir = '/Users/robmulla/Library/Group Containers/G69SCX94XU.duck/Library/Application Support/duck/Volumes/192.168.0.62 – SFTP/Repos/kaggle-nfl'

pi = pd.read_csv('{}/input/play_information.csv'.format(remote_dir))
# pi = pi.iloc[::-1]

for row in tqdm(pi.iterrows()):
    start = timer()
    year = int(row[1]['Season_Year'])
    gamekey = int(row[1]['GameKey'])
    playid = int(row[1]['PlayID'])

    print('Running for {} {} {}'.format(year, gamekey, playid))
    try:
        # print('Checking file: {}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.csv'.format(remote_dir, year, gamekey, playid))

        exists = os.path.isfile('{}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.csv'.format(remote_dir, year, gamekey, playid))
        if exists:
            print('Results already exist... skipping')
            continue
        temp_exists = os.path.isfile('{}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.temp'.format(remote_dir, year, gamekey, playid))
        if temp_exists:
            print('another worker is processing... skipping')
            continue

        touch('{}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.temp'.format(remote_dir, year, gamekey, playid))

        play = pd.read_csv('{}/working/playlevel/all_data/{}-{}-{}.csv'.format(remote_dir, year,
                                                                                  gamekey,
                                                                                  playid))
        ...
        play_danger_df.to_csv('{}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.csv'.format(remote_dir, year, gamekey, playid))
        play_danger_df.to_parquet('{}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.parquet'.format(remote_dir, year, gamekey, playid))
        end = timer()
        print(end - start)

        os.remove('{}/working/playlevel/momentum/min1yard/{}-{}-{}-lessthan1y.temp'.format(remote_dir, year, gamekey, playid))
